--[[
  ...
  The "chest" is a table that must contain the following functions:
    - chest:IsValid()  -- yes, a function with a colon
    - chest:get_output_inventory()
    - chest.localised_name -- for the title
    - chest.entity -- if presend, will create an entity preview
  Use M.wrap_entity() to wrap a real chest entity.
...
local function gui_get(player_index)
  return Globals.get_ui_state(player_index)[M.tag]
end

local function gui_set(player_index, value)
  Globals.get_ui_state(player_index)[M.tag] = value
end

function M.create(parent, player, chest)
  local self = {
    __class = "ChestInv",
    player = player,
    elems = {},
    chest = chest,
    IsValid = ChestInv.IsValid,
  }
  ...
  self.elems.scroll_pane = scroll_pane
  ...
  self.elems.header_flow = hdr

  local status_flow = hdr.add {
    type = "flow",
    direction = "horizontal",
    style = "status_flow",
  }
  ...
  self.elems.status_sprite = status_flow.add({
    type = "sprite",
    style = "status_image",
    sprite = "utility/status_working",
  })
  self.elems.status_label = status_flow.add({
    type = "label",
    style = "label",
    caption = "Working",
  })

  if chest.entity and chest.entity.valid then
    local ep_flow = scroll_pane.add({
      type = "frame",
      --direction = "vertical",
      style = "deep_frame_in_shallow_frame",
    })
    local entity_preview = ep_flow.add({
      type = "entity-preview",
    ...
    entity_preview.entity = chest.entity
  end
...
function ChestInv:IsValid()
  return self.chest:IsValid()
end
...
function ChestInv.refresh(self)
  ...
  local inv = self.chest:get_output_inventory()
  if inv == nil then
    return
  end
  local bar_idx = #inv + 1
  ...
  -- TODO: add a status callback to get the status from the chest?
  -- Or maybe the status just changes and an event is sent on_status_changed {}
  -- update the status sprite/label
  if self.status == "working" then
    self.elems.status_sprite.sprite = "utility/status_working"
    self.elems.status_label.caption = "Working"
  else
    self.elems.status_sprite.sprite = "utility/status_not_working"
    self.elems.status_label.caption = "Idle"
  end
end

function ChestInv:insert(items)
  local n_added = self.chest:insert(items)
  if n_added > 0 then
    self:refresh()
  end
  return n_added
...
local function chestinv_click_slot(self, event)
  ...
  local inv = chest:get_output_inventory()
  ...
  elseif event.button == defines.mouse_button_type.middle then
    ...
    -- TODO: if filters are enabled, then we need to pop up a filter selecter
  end
  if play_sound then
...
local function chestinv_hover_slot(self, event)
  local element = event.element
  if element == nil or element.tags.slot == nil or not self.chest:IsValid() then
    return
  end
  local inv = self.chest:get_output_inventory()
  local slot = element.tags.slot
...
function ChestWrap:IsValid()
   return self.entity ~= nil and self.entity.valid
end

function ChestWrap:get_output_inventory()
...
function M.wrap_entity(entity)
  ...
  return setmetatable(self, { __index = ChestWrap })
end

Globals.register_metaclass("ChestInv", { __index = ChestInv })
